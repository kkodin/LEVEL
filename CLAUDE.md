...
4  G2            +0                 +3            +3
5  G3            +0                 +1            +2
6  上限規格値     =基本情報!$B$6      45            45
7  社内上限規格値 =基本情報!$B$6*$B$7 36            36
8  社内下限規格値 =基本情報!$B$9*$B$8 -36           -36
9  下限規格値     =基本情報!$B$9      -45           -45
```
...
- 規格値の行は `LIMIT_SPECS` から作る。**チェックを外したものは行ごと出さない**
...
## 変位測量の規格値（`LIMIT_SPECS`）

上限・社内上限・社内下限・下限の4本。**保存ダイアログ・基本情報シート・
変位グラフシートの3か所すべてがこの1つの表から作られる。**

| key | tag | 種類 | 既定 |
|---|---|---|---|
| `upper` | `UPPER` | mm | 45 |
| `upperIn` | `UPPER_IN` | `upper` の割合 | 0.8 |
| `lowerIn` | `LOWER_IN` | `lower` の割合 | 0.8 |
| `lower` | `LOWER` | mm | -45 |

- 並びは変位グラフに出す上から下の順。**並べ替えると基本情報の行番号が動く**
- 値は `meta[key + "Value"]`、使う・使わないは `meta[key + "On"]`。
  **どちらも未設定なら既定値／「使う」**。既定値を meta に焼き込まないこと
- 社内規格値は割合だけ持ち、線の値は `元の規格値 × 割合`（`limitLineValue`）。
  親のチェックを外しても社内側だけ引ける（意図的にそうしてある）
- 保存ダイアログのチェックは `showInputModal` の `toggle` を使う。
  **チェックと入力欄を1つの `<label>` で包まないこと。** 入力欄を触るたびに反転する
...
| 6〜9 | `UPPER` / `UPPER_IN` / `LOWER_IN` / `LOWER` | B=値 / C=説明 / **D=`○`使う・`×`使わない** |
| 10 | | 空行 |
| 11 | `POINTS` | B列に「初期値」の表示名 |
| 12 | | `測点名` / `数値` / `変位グラフ` の見出し |
| 13〜 | | 登録済み基準点（C列 `○`＝変位グラフに出す / `×`＝出さない） |

**行を増減したら `XLSX_LIMIT_FIRST_ROW` / `XLSX_POINTS_ROW` を必ず直すこと。**
旧形式の `LIMIT` / `WARN` は上限規格値・社内上限規格値として読み、下限は使わない扱いにする。
...
### `rows` を書き換える経路には必ず `blockedByLock()` を通すこと

保護は誤操作から記録を守る安全装置。**キーパッド以外にも `rows` を書く経路がある。**
2026-09-02、登録済測点をタップする `recallPoint()` に保護の判定が無く、
**保護してあるのに行0の基準点と基準高が別の測点に差し替わる**事故が起きた。

`blockedByLock()` は保護中なら `rejectInput()`（画面点滅＋ブザー）で知らせて `true` を返す。
**黙って何もしないのは駄目**。押したのに何も起きないと、操作できたのか分からない。

いま通してある経路:

| 関数 | 何を書くか |
|---|---|
| `recallPoint` | 登録済測点のタップ → 行0（または対象行）の基準点 |
| `applyBasePointSelection` | 基準点選択シート → 同上 |
| `applyBaseEntry` | 基準点ドロワーの確定 → 同上 |
| `saveCurrentPoint` | 基準点追加ボタン → 同上（**台帳への登録だけは保護中も通す**） |
| `confirmPointPicker` | 測点名の選択シート → その行の測点名 |
| `#basePoint` の `input` | 基準点名の直接入力（値は `syncBaseInputs()` で戻す） |
| `deleteSavedPoint` | 台帳から消すと表の `基準高`・`誤差mm` の表示が変わる |

**変位チップ（`toggleChartPoint`）は保護中でも通す。** 野帳を書き換えないため。

`rows` を書く関数を足したら、**この表に足したうえで `blockedByLock()` を通すこと。**
